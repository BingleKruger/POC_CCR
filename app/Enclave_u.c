...
sgx_status_t ccr_poc(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_ccr_poc_t ms;
...
sgx_status_t lending_profiles(sgx_enclave_id_t eid, sgx_status_t* retval, const uint8_t* some_string_postal, size_t len_postal, const uint8_t* some_string_term, size_t len_term)
{
	sgx_status_t status;
	ms_lending_profiles_t ms;
	ms.ms_some_string_postal = some_string_postal;
	ms.ms_len_postal = len_postal;
	ms.ms_some_string_term = some_string_term;
	ms.ms_len_term = len_term;
	status = sgx_ecall(eid, 1, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t credit_health(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_credit_health_t ms;
	status = sgx_ecall(eid, 2, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t simple_ols_eg(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_simple_ols_eg_t ms;
	status = sgx_ecall(eid, 3, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t multiple_ols_eg(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_multiple_ols_eg_t ms;
	status = sgx_ecall(eid, 4, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t time_series_eg(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_time_series_eg_t ms;
	status = sgx_ecall(eid, 5, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t panel_data_eg(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_panel_data_eg_t ms;
	status = sgx_ecall(eid, 6, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t seal_sgxdata_eg(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_seal_sgxdata_eg_t ms;
	status = sgx_ecall(eid, 7, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t seal_sgxfile_eg(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_seal_sgxfile_eg_t ms;
	status = sgx_ecall(eid, 8, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t unseal_sgxfile_eg(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_unseal_sgxfile_eg_t ms;
	status = sgx_ecall(eid, 9, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t seal_sgxfile_ccr(sgx_enclave_id_t eid, sgx_status_t* retval)
{
	sgx_status_t status;
	ms_seal_sgxfile_ccr_t ms;
