...
typedef struct ms_ccr_poc_t {
	sgx_status_t ms_retval;
} ms_ccr_poc_t;

typedef struct ms_lending_profiles_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_some_string_postal;
	size_t ms_len_postal;
	const uint8_t* ms_some_string_term;
	size_t ms_len_term;
} ms_lending_profiles_t;

typedef struct ms_credit_health_t {
	sgx_status_t ms_retval;
} ms_credit_health_t;

typedef struct ms_simple_ols_eg_t {
	sgx_status_t ms_retval;
} ms_simple_ols_eg_t;

typedef struct ms_multiple_ols_eg_t {
	sgx_status_t ms_retval;
} ms_multiple_ols_eg_t;

typedef struct ms_time_series_eg_t {
	sgx_status_t ms_retval;
} ms_time_series_eg_t;

typedef struct ms_panel_data_eg_t {
	sgx_status_t ms_retval;
} ms_panel_data_eg_t;

typedef struct ms_seal_sgxdata_eg_t {
	sgx_status_t ms_retval;
} ms_seal_sgxdata_eg_t;

typedef struct ms_seal_sgxfile_eg_t {
	sgx_status_t ms_retval;
} ms_seal_sgxfile_eg_t;

typedef struct ms_unseal_sgxfile_eg_t {
	sgx_status_t ms_retval;
} ms_unseal_sgxfile_eg_t;

typedef struct ms_seal_sgxfile_ccr_t {
	sgx_status_t ms_retval;
} ms_seal_sgxfile_ccr_t;
...
static sgx_status_t SGX_CDECL sgx_ccr_poc(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ccr_poc_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ccr_poc_t* ms = SGX_CAST(ms_ccr_poc_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = ccr_poc();


	return status;
}

static sgx_status_t SGX_CDECL sgx_lending_profiles(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_lending_profiles_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_lending_profiles_t* ms = SGX_CAST(ms_lending_profiles_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_some_string_postal = ms->ms_some_string_postal;
	size_t _tmp_len_postal = ms->ms_len_postal;
	size_t _len_some_string_postal = _tmp_len_postal;
	uint8_t* _in_some_string_postal = NULL;
	const uint8_t* _tmp_some_string_term = ms->ms_some_string_term;
	size_t _tmp_len_term = ms->ms_len_term;
	size_t _len_some_string_term = _tmp_len_term;
	uint8_t* _in_some_string_term = NULL;

	CHECK_UNIQUE_POINTER(_tmp_some_string_postal, _len_some_string_postal);
	CHECK_UNIQUE_POINTER(_tmp_some_string_term, _len_some_string_term);
	...
	if (_tmp_some_string_postal != NULL && _len_some_string_postal != 0) {
		if ( _len_some_string_postal % sizeof(*_tmp_some_string_postal) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_some_string_postal = (uint8_t*)malloc(_len_some_string_postal);
		if (_in_some_string_postal == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_some_string_postal, _len_some_string_postal, _tmp_some_string_postal, _len_some_string_postal)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_some_string_term != NULL && _len_some_string_term != 0) {
		if ( _len_some_string_term % sizeof(*_tmp_some_string_term) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_some_string_term = (uint8_t*)malloc(_len_some_string_term);
		if (_in_some_string_term == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_some_string_term, _len_some_string_term, _tmp_some_string_term, _len_some_string_term)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	ms->ms_retval = lending_profiles((const uint8_t*)_in_some_string_postal, _tmp_len_postal, (const uint8_t*)_in_some_string_term, _tmp_len_term);

err:
	if (_in_some_string_postal) free(_in_some_string_postal);
	if (_in_some_string_term) free(_in_some_string_term);
	return status;
}

static sgx_status_t SGX_CDECL sgx_credit_health(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_credit_health_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_credit_health_t* ms = SGX_CAST(ms_credit_health_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = credit_health();


	return status;
}

static sgx_status_t SGX_CDECL sgx_simple_ols_eg(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_simple_ols_eg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_simple_ols_eg_t* ms = SGX_CAST(ms_simple_ols_eg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = simple_ols_eg();


	return status;
}

static sgx_status_t SGX_CDECL sgx_multiple_ols_eg(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_multiple_ols_eg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_multiple_ols_eg_t* ms = SGX_CAST(ms_multiple_ols_eg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = multiple_ols_eg();


	return status;
}

static sgx_status_t SGX_CDECL sgx_time_series_eg(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_time_series_eg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_time_series_eg_t* ms = SGX_CAST(ms_time_series_eg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = time_series_eg();


	return status;
}

static sgx_status_t SGX_CDECL sgx_panel_data_eg(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_panel_data_eg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_panel_data_eg_t* ms = SGX_CAST(ms_panel_data_eg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = panel_data_eg();


	return status;
}

static sgx_status_t SGX_CDECL sgx_seal_sgxdata_eg(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_seal_sgxdata_eg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_seal_sgxdata_eg_t* ms = SGX_CAST(ms_seal_sgxdata_eg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = seal_sgxdata_eg();


	return status;
}

static sgx_status_t SGX_CDECL sgx_seal_sgxfile_eg(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_seal_sgxfile_eg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_seal_sgxfile_eg_t* ms = SGX_CAST(ms_seal_sgxfile_eg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = seal_sgxfile_eg();


	return status;
}

static sgx_status_t SGX_CDECL sgx_unseal_sgxfile_eg(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_unseal_sgxfile_eg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_unseal_sgxfile_eg_t* ms = SGX_CAST(ms_unseal_sgxfile_eg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = unseal_sgxfile_eg();


	return status;
}

static sgx_status_t SGX_CDECL sgx_seal_sgxfile_ccr(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_seal_sgxfile_ccr_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_seal_sgxfile_ccr_t* ms = SGX_CAST(ms_seal_sgxfile_ccr_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = seal_sgxfile_ccr();
...
SGX_EXTERNC const struct {
	...
	13,
	{
		{(void*)(uintptr_t)sgx_ccr_poc, 0, 0},
		{(void*)(uintptr_t)sgx_lending_profiles, 0, 0},
		{(void*)(uintptr_t)sgx_credit_health, 0, 0},
		{(void*)(uintptr_t)sgx_simple_ols_eg, 0, 0},
		{(void*)(uintptr_t)sgx_multiple_ols_eg, 0, 0},
		{(void*)(uintptr_t)sgx_time_series_eg, 0, 0},
		{(void*)(uintptr_t)sgx_panel_data_eg, 0, 0},
		{(void*)(uintptr_t)sgx_seal_sgxdata_eg, 0, 0},
		{(void*)(uintptr_t)sgx_seal_sgxfile_eg, 0, 0},
		{(void*)(uintptr_t)sgx_unseal_sgxfile_eg, 0, 0},
		{(void*)(uintptr_t)sgx_seal_sgxfile_ccr, 0, 0},
